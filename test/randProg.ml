...
let rec get_elem state min max (n : float) (elems : (condition * probability * 'a creator) list) =
  match elems with
  | h :: ((_, p, _) :: _ as t) ->
      if n >= min && n <= max then h else get_elem state max (max +. p state) n t
  | h :: _ ->
      if n >= min && n <= max then h else failwith "get_elem: invalid input"
  | [] ->
...
let isArray (typ : type_) _ = match typ with {t= STComposed ("array", _); _} -> true | _ -> false

let arrayTypeAndSize (typ : type_) =
  match typ with {t= STComposed ("array", [t; {t= STSize n; _}]); _} -> (t, n) | _ -> failwith "not an array"

let isArrayOfType (subtype : type_) (typ : type_) =
  match typ with {t= STComposed ("array", [t1; {t= STSize _; _}]); _} -> compare subtype t1 = 0 | _ -> false

let isTuple (typ : type_) _ = match typ with {t= STComposed ("tuple", _); _} -> true | _ -> false

let tupleTypes (typ : type_) =
  match typ with {t= STComposed ("tuple", elems); _} -> elems | _ -> failwith "tupleTypes: invalid input"
...
let newBuiltinFunNoArgs state =
  pick_one state
    [(always, normal_p, fun _ -> {id= "eps"; n= None; loc}); (always, normal_p, fun _ -> {id= "pi"; n= None; loc})]
...
let newFunction () =
  ...
  let def = {next= None; name; generic_params= []; args= []; param_order= []; t= None; loc; tags= []; body} in
